...
UPLOAD_FOLDER_FOR_OUTPUT = "11"   # נניח שמוגדרת בימות להשמיע 000.wav
...
async def process_yemot_recording(audio_file_path):
    ...
    default_api_path = f"/{UPLOAD_FOLDER_FOR_OUTPUT}"  # '/11'

    if not stock_data:
        # אין CSV: בכל זאת נעביר לשלוחה 11 (ייתכן שלא יהיה מה להשמיע)
    ...
    if best_match_key:
        ...
        # --- אחרת: בונים טקסט להשמעה (ועדיין נעביר לשלוחה 11 בתשובת שרת) ---
    ...
    # ===== תמיד: מקרה ללא שלוחה ייעודית → נעלה קובץ 000.wav לשלוחה 11 ונחזיר go_to_folder=/11 =====
        ...
        if await create_audio_file_from_text(response_text, TEMP_MP3_FILE):
            if convert_mp3_to_wav(TEMP_MP3_FILE, output_yemot_wav_name):
                ...
                if not generated_audio_success:
                    log.error("כשל בהעלאת קובץ השמע לשלוחה 11.")
            else:
    ...
    # גם אם העלאת ה-WAV נכשלה — עדיין נעביר את המאזין לשלוחה 11 (לפי הבקשה)
    if not generated_audio_success:
        log.warning("מעביר לשלוחה 11 למרות כשל ביצירת/העלאת קובץ.")
